...
// Letterbox resize
cv::Mat preprocessor::letterbox(const cv::Mat& src,
                                  cv::Size        dst_size,
                                  letterbox_info*  info,
                                  cv::Scalar      pad_color)
{
...
// BGR/RGB ->CHW normalised float blob
                                               ...
                                               const norm_params& norm)
{
    ...
    int h = rgbs[0].rows, w = rgbs[0].cols, c = 3;
    int n = (int)rgbs.size();
    std::vector<float> batch(n * c * h * w);
...
// Unletterbox: map normalised bbox to original image normalised coords
cv::Rect2f preprocessor::unletterbox(const cv::Rect2f&    norm_box,
                                      const letterbox_info& info)
{
    // norm_box is already in pixel coords relative to the letterboxed image,
    // we need to subtract padding then divide by scale.
    // If the caller already normalised to [0,1], multiply back first.

    float x  = (norm_box.x - info.pad_left) / info.scale / info.orig_size.width;
    float y  = (norm_box.y - info.pad_top)  / info.scale / info.orig_size.height;
    float bw = norm_box.width  / info.scale / info.orig_size.width;
    float bh = norm_box.height / info.scale / info.orig_size.height;
